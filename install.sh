...
os_check(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if  [ "$dist_version" != "22.04" ] && [ "$dist_version" != "20.04" ]; then
            output "Неподдерживаемая версия Ubuntu. Поддерживаются только Ubuntu 22.04 и 20.04."
            exit 2
        fi
    elif [ "$lsb_dist" = "debian" ]; then
